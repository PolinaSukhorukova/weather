...
export const WeatherCard = ({
  ...
  useEffect(() => {
    ...
    const isNotUnique = data?.find((item) => item.id === storage?.id);

    if (!isNotUnique && storage && data.length < 5) {
      data.push(storage);
  ...
  return (
    <div
      className={(cn('weather-card',
        { 'weather-card--active': favoritesLocalStorage?.some(item => item.id === selectedCity) || storage?.id === selectedCity }
      ))}>
      <div className="weather-card__title title">
        <div className="title__wrap">
          ...
          {selectedCity && !favoritesLocalStorage?.some(item => item.id === selectedCity) && storage?.id !== selectedCity && (
            <button
